...
const socials = [
  ...
  {
    name: 'LinkedIn',
    href: 'https://linkedin.com/in/hasmunandar',
  },
];
...
export default function Footer() {
  ...
  return (
    <footer id="footer" className="w-full bg-[#0F172A] text-[#F8FAFC] font-sans border-t border-white/5 relative overflow-hidden">
      ...
      <motion.div className="relative z-10 max-w-6xl mx-auto px-6 md:px-12 pt-14 pb-10" variants={fadeUp} initial="hidden" whileInView="visible" viewport={VIEWPORT}>
        <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-[#1E293B] via-[#1a2540] to-[#0F172A] shadow-[0_32px_80px_rgba(0,0,0,0.4)]">
              ...
              {status === 'success' ? (
                ...
                <form ref={formRef} onSubmit={handleSubmit} className="flex flex-col gap-4 h-full">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <textarea name="message" required rows={5} placeholder="Tulis apapun yang ingin kamu sampaikan..." className="w-full bg-white/[0.04] border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-[#406093]/60 focus:bg-white/[0.06] transition-all duration-300 resize-none leading-relaxed" />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] font-black uppercase tracking-[0.28em] text-white/80">Nama</label>
                      <input type="text" name="name" required placeholder="Nama atau alias" className="w-full bg-white/[0.04] border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-[#406093]/60 focus:bg-white/[0.06] transition-all duration-300" />
                    </div>
                    <div className="flex flex-col gap-1.5">
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-12"><div className="h-px bg-gradient-to-r from-transparent via-white/10 to-transparent" /></div>

      <motion.div className="relative z-10 max-w-6xl mx-auto px-6 md:px-12 py-10 md:py-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-10" variants={fadeUp} initial="hidden" whileInView="visible" viewport={VIEWPORT}>
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 mb-1">
            <div className="w-2 h-2 rounded-full bg-[#10B981] relative"><div className="absolute inset-0 rounded-full bg-[#10B981] animate-ping opacity-60" /></div>
            <span className="text-[10px] font-bold uppercase tracking-widest text-[#10B981]">Terbuka untuk Kolaborasi</span>
          </div>
          ...
          <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#F8FAFC]/20 mt-1">© 2026 PPG E-Portfolio</p>
        </div>
        ...
        <div className="flex flex-col gap-5 items-start md:items-end">
          <div className="flex items-center gap-1.5">
            ...
            {socials.map((s, i) => (
              <a key={i} href={s.href} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-2 px-3.5 py-2 rounded-xl border border-white/10 bg-white/[0.04] text-[#F8FAFC]/60 hover:text-[#93C5FD] hover:border-[#93C5FD]/30 hover:bg-[#93C5FD]/[0.06] hover:-translate-y-0.5 transition-all duration-300">
          ...
          <div className="w-full flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2 w-full">
              <a href={`mailto:${EMAIL}`} className="flex-1 flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-white/[0.04] border border-white/10 text-[#F8FAFC]/80 hover:text-[#93C5FD] hover:border-[#93C5FD]/30 hover:bg-[#93C5FD]/[0.06] transition-all duration-300 group">
              ...
              <button onClick={handleCopy} className="shrink-0 px-3.5 py-2.5 rounded-xl bg-white/[0.04] border border-white/10 text-[#F8FAFC]/50 hover:text-[#10B981] hover:border-[#10B981]/30 hover:bg-[#10B981]/[0.06] transition-all duration-300 relative">
                {copied ? (
                ...
                {copied && <span className="absolute -top-8 left-1/2 -translate-x-1/2 text-[9px] font-bold uppercase tracking-widest bg-[#10B981] text-white px-2.5 py-1 rounded-lg whitespace-nowrap shadow-lg">Tersalin</span>}
              </button>
            </div>
          </div>
        </div>
