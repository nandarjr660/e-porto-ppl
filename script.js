...
// Initialize Efek Teks Mengetik (Typed.js)
if (document.querySelector(".typed-text")) {
  new Typed(".typed-text", {
    strings: ["S.Pd, Gr"],
    typeSpeed: 80,
    backSpeed: 50,
    backDelay: 3000, // Diam 3 detik sebelum mengetik ulang
...
function updateIndicators(activeIndex = null) {
  ...
  const itemWidth = 320;
  const currentIndex =
    activeIndex !== null ? activeIndex : Math.round(scrollLeft / itemWidth);
...
// Animate progress bars and counters when section is visible
  ...
  entries.forEach((entry) => {
    if (entry.isIntersecting) {
      ...
      counters.forEach((counter) => {
        ...
        const increment = target / 50; // Kecepatan hitungan (semakin besar pembagi semakin halus lambat)
      ...
      // Unobserve setelah animasi jalan agar tidak terulang jika di-scroll lagi
...
const fadeElements = document.querySelectorAll(
  ".artefak-card, .penilaian-card, .competency-card, .galeri-item",
);
